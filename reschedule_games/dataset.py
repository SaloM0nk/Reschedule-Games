...
def main(
    ...
    # filter for own_team and all opponent teams
    df_match = df.filter(
        ((pl.col("home_team") == own_team) & (pl.col("guest_team").is_in(opp_teams)))
        | ((pl.col("home_team").is_in(opp_teams)) & (pl.col("guest_team") == own_team))
    )
    logger.info(f"Found {df_match.shape[0]} matches between {own_team} and {', '.join(opp_teams)}.")
    ...
    for team in opp_teams:
        opponent_original_days[team] = df_match.filter(
            ((pl.col("home_team") == own_team) & (pl.col("guest_team") == team))
            | ((pl.col("home_team") == team) & (pl.col("guest_team") == own_team))
        ).select(pl.col("date")).to_series().to_list()[0].strftime("%A")
    consider_same_days = False
    if consider_days == ["SAME"]:
        consider_same_days = True
        # use the weekday(s) of the original matchup(s)
        original_match_days = sorted(
            {
                match_date.strftime("%A")
                for match_date in df_match.select(pl.col("date")).to_series().to_list()
            }
        )
        consider_days = original_match_days
        logger.info(
            f"Considering only the same day(s) of the week as the original match(es): {', '.join(original_match_days)}."
        )
    ...
    for date in available_dates.select(pl.col("date")).to_series().to_list():
        ...
        # check for own_team and opponent team matches in the same week, there may be multiple matches in the same week, so we will store them as a list
        ...
        opponent_week_matches = []
        for team in opp_teams:
            if consider_same_days:
                # if we are considering only the same day of the week as the original match, check if the current date is on the same day of the week as the original match for this opponent team
                if date.strftime("%A") != opponent_original_days[team]:
                    continue
            team_matches_in_week = df_opponent_matches_by_team[team].filter(
                (pl.col("date") >= week_start) & (pl.col("date") <= week_end)
            ).select(pl.col("date")).to_series().to_list()
            if team_matches_in_week:
                opponent_week_matches.append(
                    f"{team}: {', '.join([d.strftime('%A, %d.%m.%Y') for d in team_matches_in_week])}"
                )
        if opponent_week_matches:
            available_dates = available_dates.with_columns(
                pl.when(pl.col("date") == date).then(
                    pl.lit(" | ".join(opponent_week_matches))
    ...
    with open(output_path, "w") as f:
        ...
        original_match_dates = df_match.select(pl.col('date')).to_series().to_list()
        ...
        f.write(f"Ursprüngliche Termine: {original_match_dates_str}\n")
        ...
        for row in available_dates.iter_rows(named=True):
            ...
            day_de = to_german_weekday_name(day_en)
            date_str = row["date"].strftime(f"{day_de}, %d.%m.%Y") # format date
            own_match_str = row["own_match"] if row["own_match"] != "None" else "Spielfrei"
            opp_match_str = row["opp_match"] if row["opp_match"] != "None" else "Spielfrei"
            f.write(f"{date_str}: {own_team}: {own_match_str} | {opp_match_str}\n")
